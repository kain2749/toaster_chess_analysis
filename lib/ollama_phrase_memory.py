...
SUMMARY_STOPWORDS = {
    ...
    "for", "with", "as", "by", "at", "from", "that", "this", "it", "its",
    "was", "were", "is", "are", "be", "been", "being",
    "you", "your", "cpu", "white", "black",
    "move", "moves", "game", "played", "playing",
    "made", "still", "ended", "position",
}


def normalize_summary_words(text: str) -> list[str]:
    text = text.lower()
...
def summary_ngrams(words: list[str], min_n: int = 2, max_n: int = 6) -> list[str]:
    phrases: list[str] = []
    ...
    for n in range(min_n, max_n + 1):
        for i in range(0, len(words) - n + 1):
            ...
            meaningful = [w for w in chunk if w not in SUMMARY_STOPWORDS]
            if len(meaningful) < 2:
                continue
...
class OllamaPhraseMemory:
        ...
        with self._connect_db() as conn:
            ...
            cur.execute(
                ...
                CREATE TABLE IF NOT EXISTS summary_avoid_phrases (
                    ...
                    KEY idx_source_seen (source, last_seen),
            ...
            cur.execute(
                """
                ALTER TABLE summary_avoid_phrases
    ...
    def recent_game_summary_texts(self, *, limit: int = 25) -> list[str]:
        with self._connect_db() as conn:
            cur = conn.cursor()
            cur.execute(
                """
    ...
    def refresh_auto_summary_cooldowns(
        self,
        *,
        ...
        summaries = self.recent_game_summary_texts(limit=summary_limit)

        counter = Counter()
        for summary in summaries:
            words = normalize_summary_words(summary)
            counter.update(summary_ngrams(words, min_n=2, max_n=6))
        ...
        for phrase, count in counter.most_common():
            ...
            candidates.append((
                phrase,
                f"auto cooldown: appeared {count} times in last {summary_limit} summaries",
                "auto",
                count,
            ))
        ...
        with self._connect_db() as conn:
            ...
            cur.executemany(
                """
                INSERT INTO summary_avoid_phrases
                    (phrase, reason, source, hit_count, last_seen)
                VALUES
    ...
    def summary_phrase_avoidance_block(self, *, limit: int = 20) -> str:
        self.refresh_auto_summary_cooldowns(
            summary_limit=25,
            min_count=2,
            max_phrases=30,
        )
        ...
        if not phrases:
            return "No summary phrase cooldowns active."
